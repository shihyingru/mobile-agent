...
internal fun buildCategorizerUserMessage(
    ...
    appendLine()
    appendLine("Task:")
    appendLine("1. Name the post's specific subject, then assign 1–3 categories that")
    appendLine("   capture it concretely (e.g. \"Korea travel\", \"AI research\", \"Recipes\").")
    appendLine("2. Reuse an existing category ONLY when it genuinely matches that subject —")
    appendLine("   treat the `use when` hints as fuzzy guidance (synonyms, related concepts,")
    appendLine("   and other-language posts that mean the same thing all count). Do NOT")
    appendLine("   stretch a loosely-related existing tag just to avoid creating a new one.")
    appendLine("3. When no existing category fits the subject well, CREATE a new one — this")
    appendLine("   is expected and encouraged, not a rare exception. New names must be 3–15")
    appendLine("   characters total (letters / digits / spaces only, no more than 2 words),")
    appendLine("   concrete and easy to find later.")
    appendLine("4. \"Misc\" is the last-resort fallback. Use it ONLY when the post has no")
    appendLine("   identifiable subject at all — NEVER as a stand-in for a specific topic")
    appendLine("   you could name in 1–2 words.")
    appendLine("5. Write a 1-sentence summary of why this post mattered enough to save.")
    appendLine()
