...
- **Notion DB schema convention.** `Task` (title) · `Priority` (select; filter on `High`, mapping `Medium`/`Low` from the same select) · `Estimated Time` (number, optional → defaults to 0). Extra props (`Date`, `Area`, …) are ignored.
