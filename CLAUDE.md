...
- **Last briefing persisted in `TokenStore`, not a separate file.** Step 4's worker writes the resulting `Briefing` as JSON into the same EncryptedSharedPreferences. Task titles can be personal — keep them encrypted at rest like the credentials. `AgentRepository.getLastBriefing()` returns null on parse failure so a schema change to `Task`/`Briefing` just re-runs the agent instead of crashing.
